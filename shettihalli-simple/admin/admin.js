...
const STORAGE_VER     = "v3"; // must match app.js
...
function loadData() {
  ...
  // Migrate old products missing status field
...
function renderOverview() {
  ...
  document.getElementById("overviewTable").innerHTML = `<div class="table-wrap"><table>
    ...
    <tbody>${products.map(p=>`<tr>
      ...
      <td>${p.stockQty}</td><td>${statusPill(p)}</td>
    ...
    <tbody>${list.map(p=>`<tr>
      ...
      <td>
        <select onchange="inlineUpdate('${p.id}','status',this.value)" class="inline-select">
          ...
          <option value="sold_out" ${(!p.inStock||p.stockQty===0)&&p.status!=="coming_soon"?"selected":""}>❌ Sold Out</option>
        </select>
      </td>
...
function updateOrderStatus(i, status) { orders[i].status = status; saveOrdersData(); toast("✅ Updated"); }
function deleteOrder(i) { orders.splice(i,1); saveOrdersData(); renderOrdersTable(); toast("🗑 Removed"); }

function exportJSON() {
  const blob = new Blob([JSON.stringify(products,null,2)], {type:"application/json"});
  const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = "shettihalli-products.json"; a.click();
  toast("📤 Downloaded!");
}
